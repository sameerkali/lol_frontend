...
export default function CustomerPage() {
  ...
  const Lookup = (
    <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 20, flex: 1 }}>
      ...
      <Button
        size="lg"
        fullWidth
        icon={<Icon name="arrow-right" size={21} />}
        wobble
  ...
  const Signup = (
    <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 18, flex: 1 }}>
      ...
      <Button size="lg" fullWidth onClick={() => { setVisits(2); setPrev(0); setScreen("card"); }}>
        Create my card
      </Button>
    </div>
  );
  ...
  return (
    ...
    >
      <TopBar
        ...
        right={
          screen !== "lookup" ? (
            <Badge tone="neutral" size="sm">{visits} visits</Badge>
          ) : null
        }
      />
      ...
      {screen === "card" && tab === "card" ? (
        <div style={{ padding: "14px 20px 16px", borderTop: "var(--border)", background: "var(--paper-000)" }}>
          <Button size="lg" fullWidth icon={<Icon name="hand" size={21} />} onClick={() => openPin("visit")}>
            Mark my visit
          </Button>
        </div>
      ) : null}
      ...
      <Celebration
        open={!!celebrate}
        title={celebrate ? `${celebrate.label} unlocked!` : ""}
