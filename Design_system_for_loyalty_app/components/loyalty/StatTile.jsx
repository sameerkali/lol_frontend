...
const TONES = {
  paper: 'var(--paper-000)', grape: 'var(--grape-100)', mint: 'var(--mint-100)',
  sun: 'var(--sun-100)', coral: 'var(--coral-100)', sky: 'var(--sky-100)',
};

export function StatTile({ label, value, delta, icon, tone = 'paper', style }) {
  const up = typeof delta === 'string' && delta.trim().startsWith('+');
  return (
    <div style={{
      background: TONES[tone] || TONES.paper, border: 'var(--border)',
      borderRadius: 'var(--radius-lg)', boxShadow: 'var(--pop-1)', padding: 20,
      ...
      <div style={{ font: '700 38px/1 var(--font-mono)', color: 'var(--text-strong)' }}>{value}</div>
      {delta ? (
