...
export function Toast({ children, tone = 'success', onDismiss, style }) {
  const t = TONES[tone] || TONES.info;
  return (
    <div style={{
      display: 'inline-flex', alignItems: 'center', gap: 12,
      background: t.background, color: 'var(--ink-900)',
      border: 'var(--border)', borderRadius: 'var(--radius-pill)',
      boxShadow: 'var(--pop-2)', padding: '14px 20px',
      ...
      <span>{children}</span>
      {onDismiss ? <span onClick={onDismiss} style={{ cursor: 'pointer', display: 'inline-flex' }}><Icon name="x" size={16} /></span> : null}
    </div>
  );
}
